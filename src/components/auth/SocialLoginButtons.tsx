...
import { PENDING_USER_TYPE_KEY, normalizeUserType, type UserType } from '@/types/user';
    ...
    try {
      ...
      if (oauthError) {
        ...
        if (oauthError.message.includes('provider is not enabled')) {
          ...
          setError(
            'Redirect URL mismatch. Add this exact URL in Supabase Auth → URL Configuration: ' +
              redirectUrl
          );
        } else {
